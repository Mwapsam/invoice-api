class Api::V1::SessionsController < ApplicationController
  ...
  def single_session
    ...
    if user&.authenticate(params[:password])
      token = issue_token(user)
      render json: { user:, jwt: token, message: 'User logged in successfully' }, status: :ok
    else
      user = User.new(phone_number: customer.phone_number, password: params[:password])

      if user.save
        token = issue_token(user)
        render json: { user:, jwt: token, message: 'User created and logged in successfully' }, status: :created
      else
